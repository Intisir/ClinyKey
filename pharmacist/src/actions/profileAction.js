...
export const getTestsSuggestion = tests => dispatch => {
  axios.post("/api/scraper/tests", { tests }).then(res => {
    if (Object.keys(res.data).length > 0) {
      dispatch({
        type: GET_TESTS,
        payload: res.data
...
export const savePrescription = PrescriptionData => dispatch => {
  console.log(PrescriptionData);

  axios
    .post("/api/prescription/addPrescription", { PrescriptionData })
    .then(res => {
      if (Object.keys(res.data).length > 0) {
        dispatch({
          type: SAVE_PRES,
...
export const getPrescriptions = userData => dispatch => {
  dispatch(setProfileLoading());
  console.log(userData);

  axios
    .post("/api/prescription", { phone: userData })
    .then(res =>
      dispatch({
...
export const getProfileByOid = (oid, history, location) => dispatch => {
  dispatch(setProfileLoading());
  axios
    .get(`/api/doctor/pharmaprofile/${oid}`)
    .then(res => {
      if (Object.keys(res.data).length > 0) {
